...
test('hidden ad slots wait for width and initialize once across repeated observer events',()=>{
 let width=0,intersect,resize;const slot={getBoundingClientRect:()=>({width}),getClientRects:()=>width?[{}]:[],closest:()=>({classList:{add(){}}})};
...
test('ad queue failure is contained instead of crashing the tool page',()=>{
 let hidden=false;const slot={getBoundingClientRect:()=>({width:300}),getClientRects:()=>[{}],closest:()=>({classList:{add(){hidden=true}}})};
