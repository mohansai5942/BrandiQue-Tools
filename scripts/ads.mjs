...
const unit=(kind,extra='')=>{
 ...
 return `<aside class="ad-placement ad-${kind} ${extra}" aria-label="Advertisements"><span class="ad-label">Advertisements</span><template data-ad-template><ins class="adsbygoogle" style="display:block;${kind==='article'?'text-align:center;':''}" data-ad-client="${client}" ${extra.includes('ad-rail')?attrs.display.replace('data-ad-format="auto" data-full-width-responsive="true"','data-ad-format="vertical"'):attrs[kind]}></ins></template></aside>`;
};
async function walk(dir){for(const e of await readdir(dir,{withFileTypes:true})){const p=`${dir}/${e.name}`;if(e.isDirectory()){await walk(p);continue}if(!p.endsWith('.html'))continue;let h=await readFile(p,'utf8');
 ...
 if(isTool){
  // Keep the workspace first. Side units occupy outer gutters only on wide screens.
  h=h.replace(/(<section class="wrap(?: work)?">)(<article class="tool"[\s\S]*?<\/article>)(<\/section>)/,(_,start,body,end)=>`<section class="wrap ad-workspace">${body}${unit('display','ad-rail ad-rail-left')}${unit('display','ad-rail ad-rail-right')}${end}<div class="wrap">${unit('display','ad-banner ad-after-tool')}</div>`);
  h=h.replace(/(<section class="section wrap cols">)([\s\S]*?)(<\/section>)/,(_,start,body,end)=>start+body+unit('article','ad-editorial')+end);
