...
Both `/sitemap.xml` and `/sitemap.txt` contain the same 52 canonical URLs. Explicit content types and revalidation headers are supplied via Cloudflare `_headers`. The text sitemap is a supported alternative, not a change to canonical URLs. Submit one sitemap, not both. A build check validates contents but does not establish Google's ability to fetch the production URL. If Search Console still reports Couldn't fetch, open the failed sitemap details, run URL Inspection > Test live URL, and inspect Page availability. Check the Cloudflare security event for that request before changing any security rule; do not disable security globally. Preserve the existing Google verification token.
